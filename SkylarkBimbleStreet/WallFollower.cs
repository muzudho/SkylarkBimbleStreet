...
///     - ［現壁］：　［入力進行方向ゴースト］が食い込んでおり、［壁追従進行方向ゴースト］が食い込んでいない壁。そのうち、これが［現壁］だとフラグが立っているもの。［次壁］が複数該当することがあることから、［現壁］も複数該当することがある。
///     - ［みなし現壁］：　［入力進行方向ゴースト］、［壁追従進行方向ゴースト］がともに食い込んでいない壁。そのうち、これが［現壁］だとフラグが立っているもの。
/// 
/// いくつか状態がある。
/// 
///     - ［無反応］：　［現壁］、［次壁］がともに０件の状態。
///     - ［直前］：［現壁］が０件、［次壁］が１件以上の状態。もう１フレーム、キー入力を続ければ［現壁］が１件以上の状態になり、［次壁］は０件にクリアーされる。
///     - ［正面］：［現壁］が１件以上、［次壁］が０件の状態。進行方向は壁に向かっているが、次フレームの［壁追従進行方向］は、［入力進行方向］に向かって左右のどちらかに設定される。
///     - ［壁追従］：［現壁］が１件以上、［次壁］が０件の状態。［入力進行方向ゴースト］は［現壁］に食い込んでいる。次フレームの［壁追従進行方向ゴースト］は、どの壁にも食い込んでいない。
///     </pre>
/// </summary>
