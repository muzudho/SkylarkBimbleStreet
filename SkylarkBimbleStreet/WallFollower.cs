...
/// プレイヤーの進行方向は２種類ある。
/// 
///     - ［入力進行方向］：　プレイヤーが入力している方向。例えば、プレイヤーが右を押しているなら、入力進行方向は右。
///     - ［壁追従進行方向］：　プレイヤーが壁に沿って移動する方向。例えば、プレイヤーが右を押していて、右側に壁がある場合、壁追従進行方向は上または下になる。［壁追従］中は、［壁追従進行方向］がプレイヤーの進行方向になる。
///     - ［みなし触壁方向］：　［みなし触れ壁］がある方向。無いケースも有る。
/// 
/// ［先行ゴースト］という概念があり、種類がある。
/// 
///     - ［入力進行方向ゴースト］：　［入力進行方向］に１フレーム進んだ先に位置するゴースト（プレイヤーのコピー）。ただし、画面表示は見やすさを優先して４フレーム先の位置を表示する。
///     - ［壁追従進行方向ゴースト］：　［壁追従進行方向］に１フレーム進んだ先に位置するゴースト（プレイヤーのコピー）。ただし、画面表示は見やすさを優先して４フレーム先の位置を表示する。
///     - ［壁追従進行方向ゴーストのみなし触壁方向ゴースト］：　［壁追従進行方向ゴースト］が［みなし触壁方向］に１フレーム進んだ先に位置するゴースト（プレイヤーのコピー）。ただし、画面表示は見やすさを優先して４フレーム先の位置を表示する。
/// 
/// 壁には種類がある。
/// 
///     - ［次壁］：　［入力進行方向ゴースト］が食い込んだ壁。複数該当することもある。
///     - ［触壁］：　［入力進行方向ゴースト］が食い込んでおり、［壁追従進行方向ゴースト］が食い込んでいない壁。そのうち、これが［みなし触壁］だとフラグが立っているもの。［次壁］が複数該当することがあることから、［触壁］も複数該当することがある。
///     - ［蹴壁］：　［入力進行方向ゴースト］、［壁追従進行方向ゴースト］がともに食い込んでいない壁。そのうち、これが［みなし触壁］だとフラグが立っているもの。
///         - ※［触壁］と［蹴壁］は排他的なので、どちらかに件数があるときは、もう一方の件数は０件になる。
///     - ［みなし触壁］：　［みなし触壁］フラグが立っているもの。
/// 
/// いくつか状態がある。
/// 
///     - ［無反応］：　［蹴壁］、［触壁］、［次壁］がともに０件の状態。
///     - ［触壁直前］：　［蹴壁］、［触壁］が０件、［次壁］が１件以上の状態。もう１フレーム、キー入力を続ければ［触壁］が１件以上の状態になり、［次壁］は０件にクリアーされる。
///     - ［触壁正面］：　［蹴壁］または［触壁］のいずれかが１件以上、［次壁］が０件の状態。進行方向は壁に向かっているが、次フレームの［壁追従進行方向］は、［入力進行方向］に向かって左右のどちらかに設定される。
///     - ［触壁追従］：　［触壁］が１件以上、［次壁］が０件の状態。次フレームの［壁追従進行方向ゴースト］は、どの壁にも食い込んでいない。
///         - ［初蹴壁直前］：　現在、［触壁］状態にあり、［壁追従進行方向ゴーストのみなし触壁方向ゴースト］がどの壁にも食い込まない状態。
///     - ［蹴壁追従］：　［蹴壁］が１件以上、［次壁］が０件の状態。次フレームの［壁追従進行方向ゴースト］は、どの壁にも食い込んでいない。
///         - ［再蹴壁直前］：　現在、［蹴壁］状態にあり、［壁追従進行方向ゴーストのみなし触壁方向ゴースト］がどの壁にも食い込まない状態。
///     </pre>
/// </summary>
