...
public class Game1 : Game
{
    ...
    {
        var contactDirection = GetWallFollowVisualContactDirection();
        if (contactDirection == Vector2.Zero)
        {
            DrawRectangle(player, bodyColor);
            DrawRectangle(Inset(player, Math.Max(6, GetCurrentPlayerSize() / 5)), innerColor);
            return player;
        }

        return SlimeRenderer.Draw(player, contactDirection, bodyColor, innerColor, CurrentPalette.GemShine, GetCurrentPlayerSize(), DrawRectangle);
    }

    private Vector2 GetWallFollowVisualContactDirection()
    {
        // 壁追従中の内部方向を優先し、なければ親壁ハイライト用の接触情報を使う。
